	...
	if(ISUPDATE2>=0)  // set every step
	{
		...
		{	
			...
			{				
				...
				double simStep = AKIGetSimulationStepTime();
				if(ln_meteringId[i][j]>0)					
				tmp_err1 = ECIChangeParametersFlowMeteringById(ln_meteringId[i][j],
					timeSta, max_RM_rt, actual_r[i][j], amin, currentTime, simStep);
			} // for j-loop end			
		}// for i-loop end	
		fprintf(rm_f, "\n");
		ISUPDATE2=0;
