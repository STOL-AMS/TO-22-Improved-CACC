...
#include <stdlib.h>
#include <iostream>
//FILE *dbg_f, *dmd_f, *rm_f, *sec_outfile, *pp, *cfg_file, *vsl_crm_f; //*det_outfile,
...
void createparamterbase(int replication, int acc, int cacc, std::string paramtersets)
{
	...
	const unsigned short *MAINLANE_PercentString = 
		...
		"through_flow");
	int total_through = (int)ANGConnGetAttributeValueDouble
	...
	const unsigned short *on_RAMP_PercentString = 
		AKIConvertFromAsciiString( 
		"on_ramp_flow");
	int on_ramp = (int)ANGConnGetAttributeValueDouble
	...
	char str_tmp3[len_str] = "a"; 
	sprintf_s(str_tmp3,len_str, "C:\\CACC_Simu_Data\\acc%u_cacc%u\\%u\\detector\\%s_%s_%s", acc, cacc, replication, paramtersets.c_str(),buffer,buffer2);
	if (dirExists(str_tmp3) == false)
	{
		_mkdir(str_tmp3);
	}
}
...
//////////////////////////////////////////////////////////////////////////
//put to the same folder as we save detector file
//////////////////////////////////////////////////////////////////////////
void CreateFileRecordACC2Manual(double acc_percent, double cacc_percent, int rep_id, std::string parastr)
{

	char str_tmp[len_str] = "a"; 
	sprintf_s(str_tmp,len_str, 
		"C:\\CACC_Simu_Data\\acc%u_cacc%u\\%u\\detector\\%s\\acc2manual.txt", 
	...
	char str_tmp1[len_str] = "a";
	sprintf_s(str_tmp1, len_str, "C:\\CACC_Simu_Data\\acc%u_cacc%u\\%s", (int)(acc_percent * 100), (int)(cacc_percent * 100), parastr.c_str());
	if (dirExists(str_tmp1) == false)
	{
		_mkdir(str_tmp1);
	}
	...
	sprintf_s(str_tmp, len_str,
		"C:\\CACC_Simu_Data\\acc%u_cacc%u\\%s\\modeSwitch_%u.txt", 
		(int)(acc_percent * 100), (int)(cacc_percent * 100), parastr.c_str(), rep_id);

	FILE* file1 = fopen(str_tmp, "w+");
	fclose(file1);
}
...
//put to the same folder as we save detector file
//////////////////////////////////////////////////////////////////////////
void CreateFileRecordCrash(double acc_percent, double cacc_percent, int rep_id, std::string parastr)
{
...
int Init_sim_data_out(double acc_percent, double cacc_percent)
{
	...
	CreateFileRecordACC2Manual(acc_percent, cacc_percent, replication, line);
	CreateFileRecordCrash(acc_percent, cacc_percent, replication, line);
...
bool Init()  // A major function; Called by AAPI.cxx: the top function for intialization of the whole system
{
	...
	InitRealtimeDetection();	// memory allocation; for detection measure
